...
# =========================================================================
# CSS — Tema Dark Profesional dengan Spacing yang Baik
# =========================================================================
st.markdown(
    """
    <style>
    ...
    /* Card Utama - Setiap Card Terpisah dengan Spacing */
    .main-card {
        background: #1E293B;
        border-radius: 12px;
    ...
    .badge-data { background: rgba(251, 191, 36, 0.15); color: #FBBF24; }
...
with st.container():
    ...
    <div class="main-card">
        <div class="card-title">
            📊 Status Terkini Mesin
            <span class="badge badge-data">REAL-TIME</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with st.container():
    ...
    <div class="main-card">
        <div class="card-title">
            🧹 Pembersihan & Verifikasi Data
            <span class="badge badge-data">DATA QUALITY</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ============================================================
    # SCATTER PLOT OUTLIER (di dalam card yang sama)
    # ============================================================
...
# =========================================================================
# CARD 4: FORECASTING (ARIMA vs LSTM) - DIPERBAIKI
# =========================================================================
with st.container():
    st.markdown("""
    <div class="main-card">
    ...
    def _buat_grafik_forecast(df_historis, forecast_arima, forecast_lstm, kolom_aktual, kolom_prediksi,
        ...
        # --- ARIMA ---
        # PERBAIKAN: Filter hanya untuk tanggal yang dipilih
        if not forecast_arima.empty:
            data_arima = forecast_arima.dropna(subset=[kolom_prediksi])
        ...
        if not forecast_lstm.empty:
            data_lstm = forecast_lstm.dropna(subset=[kolom_prediksi])
        ...
        # --- Garis penanda "mulai forecast" ---
        if not df_historis.empty and (not forecast_arima.empty or not forecast_lstm.empty):
            # Cari waktu mulai forecast (data aktual terakhir atau forecast pertama)
            waktu_aktual_terakhir = df_historis["created_at"].max()
            waktu_forecast_pertama = None
            
            if not forecast_arima.empty:
                waktu_forecast_pertama = forecast_arima["target_waktu"].min()
            elif not forecast_lstm.empty:
                waktu_forecast_pertama = forecast_lstm["target_waktu"].min()
            
            if waktu_forecast_pertama:
                fig.add_vline(
                    x=waktu_forecast_pertama,
                    line_width=2,
                    line_dash="dash",
                    line_color="#64748B",
    ...
    # Cek apakah ada data forecast
    if not all_forecast.empty and "target_waktu" in all_forecast.columns:
        ...
        if default_tanggal:
            ...
            # Jika tidak ada data historis di tanggal itu, ambil data sebelumnya
            ...
            if not forecast_arima.empty or not forecast_lstm.empty:
                ...
                with tab_suhu:
                    ...
                    with col_arima_suhu:
                        st.markdown("**ARIMA**")
                        data_arima = forecast_arima.dropna(subset=["nilai_suhu_prediksi"]) if not forecast_arima.empty else pd.DataFrame()
                        if not data_arima.empty:
                            st.dataframe(
                    ...
                    with col_lstm_suhu:
                        st.markdown("**LSTM**")
                        data_lstm = forecast_lstm.dropna(subset=["nilai_suhu_prediksi"]) if not forecast_lstm.empty else pd.DataFrame()
                        if not data_lstm.empty:
                            st.dataframe(
                ...
                with tab_getaran:
                    ...
                    with col_arima_getaran:
                        st.markdown("**ARIMA**")
                        data_arima = forecast_arima.dropna(subset=["nilai_getaran_prediksi"]) if not forecast_arima.empty else pd.DataFrame()
                        if not data_arima.empty:
                            st.dataframe(
                    ...
                    with col_lstm_getaran:
                        st.markdown("**LSTM**")
                        data_lstm = forecast_lstm.dropna(subset=["nilai_getaran_prediksi"]) if not forecast_lstm.empty else pd.DataFrame()
                        if not data_lstm.empty:
                            st.dataframe(
